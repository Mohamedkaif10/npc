...
class EnhancedPMMConfig(BaseClientModel):
    ...
    max_inventory: Decimal = Field(10, client_data=ClientFieldData(
        prompt_on_new=True, prompt=lambda mi: "Max base asset inventory"))
    ...
    sma_period: int = Field(50, client_data=ClientFieldData(
        prompt_on_new=True, prompt=lambda mi: "SMA period for trend analysis"))


class EnhancedPMM(ScriptStrategyBase):
    """
    Enhanced PMM with inventory management, trend analysis, and risk framework
    """
    ...
    def update_price_history(self):
        ...
        try:
            ...
            self.log_with_clock(logging.ERROR, f"Error updating price history: {e}")

    def calculate_sma(self):
        """Calculate SMA using stored price history"""
        try:
            if len(self.price_history) < self.config.sma_period:
                return None
            return np.mean(self.price_history[-self.config.sma_period:])
        except Exception as e:
            self.log_with_clock(logging.ERROR, f"Error calculating SMA: {e}")
            return None
    ...
    def manage_risk(self):
        ...
        if base_balance > self.config.max_inventory:
            self.log_with_clock(logging.WARNING, f"Max inventory reached! Current: {base_balance}, Max: {self.config.max_inventory}")
            return False
        
        if self.last_price > Decimal("0"):
            price_change = (current_price - self.last_price) / self.last_price
            if price_change < -float(self.config.stop_loss_pct):
                self.log_with_clock(logging.ERROR, f"Stop loss triggered! Price change: {price_change:.2%}")
                self.cancel_all_orders()
                return False
    ...
    def create_proposal(self) -> List[OrderCandidate]:
        ...
        if sma is not None:
            ...
            trend_factor = (current_price - sma) / sma if sma > 0 else 0
        ...
        self.log_with_clock(logging.INFO, f"Inventory: {inventory_pct:.2%}, Buy amount: {buy_amount}, Sell amount: {sell_amount}")
    ...
    def did_fill_order(self, event: OrderFilledEvent):
        ...
        self.entry_prices[event.order_id] = float(event.price)
